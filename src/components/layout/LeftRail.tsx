...
export function BrandMark() {
  return (
    <svg viewBox="0 0 42 42" fill="none" aria-hidden="true">
      <path
        d="M34 6H19L8 17v11l8-8h12l-6 7H10l-6 8h19l12-13V12L23 24h-8l8-9h8l7-9ZM12 31l-4-4M19 10l-7 8v5M29 28l-7 7H11"
        stroke="currentColor"
        strokeWidth="1.5"
      ...
      <nav aria-label="Main navigation">
        {groups.map((group) => (
          <section key={group.title}>
            <h2>{group.title}</h2>
            {group.items.map(({ label, icon: Icon, to, count }) =>
              to ? (
                <NavLink
                  key={label}
                  to={to}
                ...
                >
                  <Icon />
                  <span>{label}</span>
                  {!!count && <b className="rail-count">{count}</b>}
                </NavLink>
              ) : (
                <span
                  key={label}
                  className="rail-link unavailable"
